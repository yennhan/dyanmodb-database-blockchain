...
app = Flask(__name__)
Bootstrap(app)
supplier1 = Blueprint('supplier1',__name__)
...
#veggy supplier
@supplier1.route('/supplier1_homepage',methods=['GET','POST'])
def supplier_homepage():
    ...
    for i in range(len(the_restock)):
        if the_restock[i]['supplierID'] == "supplier1" and the_restock[i]['restock_status'] == "Required_restock":
            data_invoice.append(the_restock[i])
    for i in range(len(the_restock)):
        if the_restock[i]['supplierID']=="supplier1" and the_restock[i]['restock_status']=="Completed":

            things.append(the_restock[i])
    return render_template('supplier1_dashboard.html', invoice=data_invoice,data=things)


@supplier1.route('/adding_invoice_supplier1',methods=['GET','POST'])
...
@supplier1.route('/purchased',methods=['GET','POST'])
...
@supplier1.route('/issue_invoice',methods=['GET','POST'])
def issue_invoice():
